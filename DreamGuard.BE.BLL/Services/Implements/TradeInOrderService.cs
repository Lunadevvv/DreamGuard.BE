...
namespace DreamGuard.BE.BLL.Services.Implements
{
    public class TradeInOrderService : ITradeInOrderService
    {
        ...
        public async Task<Result<TradeInOrderDashBoardResponse>> GetTradeInDashBoardAsync(DateOnly fromDate, DateOnly toDate)
        {
            ...
            decimal totalDepositAmount = payments.Where(p => p.PaymentType == PaymentType.Deposit && p.Status == PaymentStatus.Paid && p.TradeInOrderId != null).Sum(p => p.Amount);
            decimal totalCODAmount = payments.Where(p => p.PaymentType == PaymentType.Purchase && p.PaymentMethod == PaymentMethod.COD && p.Status == PaymentStatus.CODPaid && p.TradeInOrderId != null).Sum(p => p.Amount);
            decimal totalRefundAmount = payments.Where(p => p.PaymentType == PaymentType.Refund && p.Status == PaymentStatus.Refunded && p.TradeInOrderId != null).Sum(p => p.Amount);
            decimal totalVnPayAmount = payments.Where(p => p.Status == PaymentStatus.Paid && p.TradeInOrderId != null && p.PaymentMethod == PaymentMethod.VnPay).Sum(p => p.Amount);
            decimal totalPurchaseAmount = payments.Where(p => p.PaymentType == PaymentType.Purchase && (p.Status == PaymentStatus.CODPaid || p.Status == PaymentStatus.Paid) && p.TradeInOrderId != null).Sum(p => p.Amount);
            ...
            var response = new TradeInOrderDashBoardResponse
            {
                ...
                TotalAmount = totalDepositAmount + totalCODAmount + totalVnPayAmount,
