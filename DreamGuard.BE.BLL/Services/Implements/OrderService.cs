    ...
    public class OrderService : IOrderService
    {
        ...
        public async Task<Result<CheckoutProductOrderResponse>> CreateOrderAsync(Guid userId, CreateOrderRequest request, string ipAddress)
        {
            ...
            try
            {
                ...
                _checkoutProductOrderRepository.AddEntity(checkoutProductOrder);
                ...
                async Task CreateChildOrder(List<OrderItem> items, string suffix)
                {
                    ...
                    _orderRepository.AddEntity(childOrder);
        ...
        public async Task<Result<OrderDetailResponse>> GetOrderByIdAsync(Guid orderId)
        {
            ...
            var checkoutPayment = checkoutOrder?.Payments.FirstOrDefault(co => co.CheckoutProductOrderId == checkoutOrder.Id);
            return Result<OrderDetailResponse>.Success(MapToDetailResponse(order, checkoutPayment));
        }
        ...
        private async Task<Result> CancelOrderInternalAsync(Order order, decimal refundAmount)
        {
            ...
            try
            {
                ...
                if (order.CheckoutProductOrderId.HasValue)
                {
                    ...
                    if (checkoutOrder != null)
                    {
                        var checkoutPayment = checkoutOrder.Payments.OrderBy(cop => cop.CreatedAt).FirstOrDefault();
                        if(checkoutPayment!.PaymentMethod == PaymentMethod.VnPay)
                        {
                        ...
                        }else if (checkoutPayment!.PaymentMethod == PaymentMethod.COD && checkoutPayment.Status == PaymentStatus.Pending)
                        {
                            ...
                            checkoutPayment.Status = PaymentStatus.Failed;
                            checkoutPayment.UpdatedAt = DateTime.UtcNow;
                            _paymentRepository.UpdateEntity(checkoutPayment);
                        }
        ...
        private static OrderDetailResponse MapToDetailResponse(Order order, Payment? checkoutPayment)
        {
            return new OrderDetailResponse
            {
                ...
                PaymentMethod = checkoutPayment?.PaymentMethod ?? PaymentMethod.Other,
                PaymentStatus = checkoutPayment?.Status ?? PaymentStatus.Pending,
